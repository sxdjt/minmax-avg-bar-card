let t,e=!1;try{t=await import("lit")}catch{t=await import("https://cdn.jsdelivr.net/npm/lit@3/+esm"),e=!0}const{LitElement:r,html:o,css:i,svg:s,nothing:a}=t;console.info(`[MMAB] v1.0.0 loaded (Lit: ${e?"CDN":"local"})`);const n={cs:{missing:"Chybí konfigurace – zadej entitu.",min:"Min",max:"Max",avg:"Průměr",thresholds:"Barevné rozsahy",thresholds_by:"podle",add:"Přidat",remove:"Odebrat",lt:"méně než",color:"barva",months:"Měsíce",weeks:"Týdny",preset:"Přednastavený styl",color_by:"Barva podle",color_by_max:"Maximum",color_by_average:"Průměr",color_by_min:"Minimum"},en:{missing:"Missing config – provide an entity.",min:"Min",max:"Max",avg:"Avg",thresholds:"Color ranges",thresholds_by:"by",add:"Add",remove:"Remove",lt:"less than",color:"color",months:"Months",weeks:"Weeks",preset:"Style Preset",color_by:"Color by",color_by_max:"Maximum",color_by_average:"Average",color_by_min:"Minimum"}},l={temperature:[{lt:-15,color:"#b968f4"},{lt:0,color:"#039be5"},{lt:20,color:"#43a047"},{lt:25,color:"#fdd835"},{lt:30,color:"#fb8c00"},{lt:999,color:"#e53935"}],temperature_f:[{lt:5,color:"#b968f4"},{lt:32,color:"#039be5"},{lt:68,color:"#43a047"},{lt:77,color:"#fdd835"},{lt:86,color:"#fb8c00"},{lt:999,color:"#e53935"}],beaufort:[{lt:1,color:"#2196F3"},{lt:5,color:"#64B5F6"},{lt:11,color:"#4DD0E1"},{lt:19,color:"#4CAF50"},{lt:28,color:"#8BC34A"},{lt:38,color:"#CDDC39"},{lt:49,color:"#FFEB3B"},{lt:61,color:"#FFC107"},{lt:74,color:"#FF9800"},{lt:88,color:"#FF5722"},{lt:102,color:"#F44336"},{lt:117,color:"#D32F2F"},{lt:999,color:"#B71C1C"}]},c=t=>String(t).padStart(2,"0"),h=(t,e,r)=>Math.min(r,Math.max(e,t)),d=(t,e)=>{const r=new Date(t);return r.setTime(r.getTime()+3600*e*1e3),r},m=(t,e)=>{const r=new Date(t);return r.setDate(r.getDate()+e),r},g=(t,e)=>{const r=new Date(t);return r.setMonth(r.getMonth()+e),r},u=["Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sep","Oct","Nov","Dec"];function p(t,e="eu"){return"intl"===e?`${t.getDate()}-${u[t.getMonth()]}`:`${t.getDate()}. ${t.getMonth()+1}.`}function b(t,e="eu"){return"intl"===e?`${t.getDate()}-${u[t.getMonth()]}-${t.getFullYear()}`:`${t.getDate()}. ${t.getMonth()+1}. ${t.getFullYear()}`}function _(t){return`${c(t.getHours())}:${c(t.getMinutes())}`}class f extends r{static get properties(){return{hass:{},_config:{},_data:{state:!0},_loading:{state:!0},_err:{state:!0},_hover:{state:!0},_size:{state:!0},_selection:{state:!0},_periodMode:{state:!0},__lastFetchKey:{state:!0}}}constructor(){super(),this._size={w:900,h:320},this._selection={startIso:"",endIso:"",wsPeriod:""},this._periodMode="month",this.__ro=null,this._energySubscription=null}static get styles(){return i`
      :host {
        display: block;
        ...
        <div class="wrap" style="--mmab-height:${x}px;">
          ...
          <div class="chart"
            ...
            <svg viewBox="0 0 ${T} ${A}" role="img" aria-label="Min max avg bar chart">
              ${U&&f?s`<text class="yUnit" x="${z-5}" y="${P-6}" text-anchor="end">${f}</text>`:a}
              
              ${W.ticks.map((t,e)=>{const r=R(t),o=0===e||e===W.ticks.length-1;return s`
                  <line class="${o?"gridHStrong":"gridH"}" x1="${z}" y1="${r}" x2="${z+I}" y2="${r}"></line>
                  ${q?s`<text class="tickText" x="${z-8}" y="${r+4}" text-anchor="end">${Number(t).toFixed(K)}</text>`:a}
                `})}
              ${(()=>{const t=[];for(let e=0;e<=L;e++)if(e%J===0){const r=z+e*O+O/2;t.push(s`<line class="gridV" x1="${r}" y1="${P}" x2="${r}" y2="${P+B}"></line>`)}return t})()}
              
              ${$.map((e,r)=>{if(e.isEmpty)return a;const o=isFinite(e.min)?e.min:null,i=isFinite(e.max)?e.max:null,n=isFinite(e.mean)?e.mean:null;if(null==o||null==i)return a;const c=z+r*O+j,h=function(t,e){if(!isFinite(t))return"var(--disabled-text-color)";const r=(Array.isArray(e)&&e.length?e:l.temperature).map(t=>({lt:Number(t.lt),color:String(t.color??"")})).filter(t=>isFinite(t.lt)&&t.color).sort((t,e)=>t.lt-e.lt);for(const e of r)if(t<e.lt)return e.color;return r.length?r[r.length-1].color:"var(--primary-color)"}("min"===rt?o:"average"===rt?n??i:i,et),d=R(i),m=R(o),g=Math.max(2,m-d),u=Number(t.bar_radius??4),p=null==n?null:R(n),b=G&&G.idx===r;return s`
